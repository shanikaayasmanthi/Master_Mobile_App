...
class _HomeState extends State<Home> {
  List<dynamic> supervisorList = [].obs;

  final RxList<String> selectedSupervisorIds = <String>[].obs;
  ...
  Future<void> loadSupervisors()async{
    final fetchData = await attendanceController.getSupervisors(company: user!.company);
    if(fetchData!=null){
      supervisorList.assignAll(fetchData);
    }
  }
                          ...
                          Obx(() {
                            return Column(
                              children: supervisorList.map((supervisor) {
                                final String supervisorId = supervisor['epf_number'] as String;
